...
/**
 * REST controller for managing {@link org.emu.membership.domain.Member}.
 */
@RestController
@RequestMapping("/api")
public class MemberResource {
    ...
    private static final String ENTITY_NAME = "membershipMember";
    ...
    /**
     * {@code PATCH  /members} : Updates given fields of an existing member.
     *
     * @param memberDTO the memberDTO to update.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated memberDTO,
     * or with status {@code 400 (Bad Request)} if the memberDTO is not valid,
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/members", consumes = "application/merge-patch+json")
    public ResponseEntity<MemberDTO> partialUpdateMember(@NotNull @RequestBody MemberDTO memberDTO) throws URISyntaxException {
        log.debug("REST request to update Member partially : {}", memberDTO);
        if (memberDTO.getId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
    ...
    @GetMapping("/_search/membersByFirstName/{key}")
    public List<Member> searchMembersContainsInFirstName(@PathVariable String key) {
        List<Member> list = memberService.searchByFirstName(key);
        return list;
    }

    @GetMapping("/_search/test/{key}")
    public List<SearchHits<?>> test(@PathVariable String key) throws ClassNotFoundException {
        List<SearchHits<?>> searchHits = memberService.searchInMultipleIdxs(key);
        return searchHits;
    }
}
