...
import org.emu.membership.config.KafkaProperties;
...
class MembershipKafkaResourceIT {
    ...
    void setup() {
        ...
        MembershipKafkaResource kafkaResource = new MembershipKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/membership-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/membership-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
